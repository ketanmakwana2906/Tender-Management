...
public class TenderDaoImpl implements TenderDao{
	    ...
	    try {
	        ...
	        if (rs.next()) {
	            do {
	                ...
	                int bexp = rs.getInt(8);
	                tender = new TenderBean(id, name, type, price, desc, deadline, location, bexp);
	...
	public List<TenderBean> getAllTenders() {
		...
		try {
			...
			while(rs.next())
			{
				...
				tender.setBexp(rs.getInt("bexperience"));
				Timestamp sqlDate = rs.getTimestamp(6);
				tender.setDeadline(sqlDate);
	...
	@Override
	public List<TenderBean> getvenTenders() {
		List<TenderBean> tenderList  = new ArrayList<TenderBean>();
		...
		try {
			ps = con.prepareStatement("SELECT t.* FROM tender t LEFT JOIN tenderstatus ts ON t.tid = ts.tid WHERE t.tdeadline > NOW() AND ts.tid IS NULL;");
	...
	public String createTender(TenderBean tender) {
		...
		try {
			...
			Timestamp deadline = tender.getDeadline();
			pst.setTimestamp(6, deadline);

			
			pst.setString(7, tender.getLocation());
			pst.setInt(8, tender.getBexp());
